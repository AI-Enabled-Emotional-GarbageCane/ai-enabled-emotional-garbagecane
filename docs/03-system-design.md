...
| **Intel RealSense D435** ★ | RGB-D 影像取得 | ◎ 採用：同時提供 RGB（給模型推論）與 depth（自動 ROI 分割、距離量測） |
| Raspberry Pi 5 (8GB) | 邊緣運算 | ✗ 不採：CPU 推論吞吐有限 |
| ESP32-CAM | 影像取得 | ✗ 不採：記憶體不足，無 depth 串流 |
| 筆電 + USB Webcam | 開發過渡 | △ 僅作為 Phase 2 開發階段使用，最終部署以 Jetson + D435 為主 |

### 決策

**採用 NVIDIA Jetson AGX Orin Nano + Intel RealSense D435** 作為正式部署之邊緣運算與感測組合。

**RealSense D435 之雙重角色**：
- **RGB 串流** → YOLOv8n 推論之輸入（由 `vision` 模組管理）。
- **Depth 串流** → (a) 偵測使用者靠近（距離 < 閾值，由 `firmware` 模組管理）；(b) 投入物之自動 ROI 分割（自桶口背景分離出物件區域）。D435 同時取代外接超音波（HC-SR04 不再採用）。

> **備案**：若 D435 深度鏡頭借不到，改用一般 USB Webcam（RGB only）+ 外接超音波感測器（HC-SR04）替代距離偵測功能。

**開發流程**：
- 模型訓練於 Google Colab（T4 GPU，免費），權重輸出為 ONNX。
- ONNX 於 Jetson 上轉換為 TensorRT engine 後執行推論。
- D435 透過 librealsense（Jetson 上 USB 3.0 介接）提供 RGB-D 串流。D435 對 Jetson AGX Orin 相容性良好，可直接使用 CUDA Runtime。
...
| Queue | 方向 | Payload | 說明 |
|---|---|---|---|
| `q_detected` | firmware → vision | `{ "event": "user_detected", "distance_cm": <number>, "ts": <iso8601> }` | D435 depth 偵測到使用者靠近 |
...
| 模組 | 職責 | 不負責 |
|---|---|---|
| `vision` | 模型訓練、即時推論、回傳 `{class, confidence, num_objects, snapshot_path}`；D435 RGB 串流管理 | 不負責語音、不負責 UI |
| `firmware` | D435 depth 串流 → 距離感測、LED 燈號控制 | 不負責推論、不負責語音 |
