...
| TBD | **Firmware／感測** | D435 depth 距離感測；LED 燈號；透過 Queue 送 `user_detected` 給 vision | `firmware` |
...
| 全員 | 確認硬體取得方案（Jetson AGX Orin Nano 已確定；D435 若借不到則備案 USB Webcam + HC-SR04） |
...
| Firmware | D435 depth 距離感測可運作；LED 控制可運作；可透過 Queue 送 `user_detected` |
...
| Firmware | D435 depth 感測與 vision RGB 串流同時運行的穩定性測試；LED 固定在桶子上 |
...
| D435 深度鏡頭借不到 | 中 | 中 | 備案：USB Webcam（RGB only）+ HC-SR04 超音波替代距離偵測 |
...
2. **下週**：確認硬體取得（Jetson AGX Orin Nano 已確定；D435 借用或備案）。
